# Run missing PHROG analysis models
library(glmmTMB)
library(broom.mixed)
library(dplyr)
library(purrr)
...
# Create PA data and long format
...
# Run subset for testing (first 100 PHROGs)
phrog_subset <- unique(total_pa_long$phrog_id)[1:100]

# PA Model 4: GEE
cat("Running PA Model 4: GEE subset...\n")
total_gee_subset_results <- total_pa_long %>%
...
write.csv(total_gee_subset_results, "../Orf_Contig_Phrog_compositional/phrog_results/total_PA_GEE_subset_results.csv", row.names = FALSE)
cat("Completed PA Model 4 subset - Results rows:", nrow(total_gee_subset_results), "\n")

# Abundance Model 2: Hurdle Negative Binomial GLMM
cat("Running Abundance Model 2: Hurdle NB subset...\n")
total_hurdle_subset_results <- total_abundance_long %>%
  filter(phrog_id %in% phrog_subset) %>%
  group_by(phrog_id) %>%
  nest() %>%
  mutate(
...
write.csv(total_hurdle_subset_results, "../Orf_Contig_Phrog_compositional/phrog_results/total_abundance_hurdle_subset_results.csv", row.names = FALSE)
cat("Completed Abundance Model 2 subset - Results rows:", nrow(total_hurdle_subset_results), "\n")

# Abundance Model 3: Zero-Inflated Negative Binomial GLMM
cat("Running Abundance Model 3: Zero-Inflated NB subset...\n")
total_zinb_subset_results <- total_abundance_long %>%
  filter(phrog_id %in% phrog_subset) %>%
  group_by(phrog_id) %>%
  nest() %>%
  mutate(
...
write.csv(total_zinb_subset_results, "../Orf_Contig_Phrog_compositional/phrog_results/total_abundance_zinb_subset_results.csv", row.names = FALSE)
cat("Completed Abundance Model 3 subset - Results rows:", nrow(total_zinb_subset_results), "\n")

cat("All missing models completed successfully!\n")
...
check <- contig.abundance.table_0.75_prevFiltered %>% select("Row.names",26:207) %>% column_to_rownames("Row.names")

length(rowSums(check)[rowSums(check) == 0]) # 39
all_0_samples <- names(rowSums(check)[rowSums(check) == 0])
length(colSums(check)[colSums(check) == 0]) # 0

contig.abundance.table_0.75_prevFiltered <- contig.abundance.table_0.75_prevFiltered %>%
                                            filter(!Row.names %in% all_0_samples)
# 255 samples x 207 metadata+contigs
...
model.design <- model.matrix(~ Dx.Status * ns(month,df=3) + Country + Sex + ns(as.numeric(Age.at.Gluten.Introduction..months.),df=3) + Delivery.Mode + HLA.Category + feeding_first_year,metadata.clean)
...
model.res <- topTable(model.fit,coef="Dx.StatusCELIAC:ns(month, df = 3)1",number=Inf)
sig.model.res <- topTable(model.fit,coef="Dx.StatusCELIAC:ns(month, df = 3)1",number=Inf) %>% filter(adj.P.Val < 0.05)
dim(sig.model.res)
# 12 significant contig

# COMPLETE CONTIG-BASED ANALYSIS: ALL 9 MODELS × ALL 3 COHORTS
# Based on Celiac_phage_orf_contig_phrog_PA_abundance_analysis.Rmd
# Total: 5 PA Models + 4 Abundance Models = 9 Models
# All 3 Cohorts: Total, US, Italy
# Output: /Orf_Contig_Phrog_compositional/results/ and /figures/
...
cat("=== EXECUTING ALL 9 CONTIG MODELS FOR ALL 3 COHORTS ===\n")
...
dir.create("Orf_Contig_Phrog_compositional/results", recursive = TRUE, showWarnings = FALSE)
dir.create("Orf_Contig_Phrog_compositional/figures", recursive = TRUE, showWarnings = FALSE)

# Data loading function for contigs
load_contig_cohort_data <- function(cohort_name) {
  cat("Loading", cohort_name, "contig cohort data...\n")
  
  # Load abundance data
  abundance_file <- paste0("Contig/data/", tolower(cohort_name), "/", cohort_name, ".contig.abundance.clean_0.75_0.03.csv")
  abundance_data <- read.csv(abundance_file) %>% column_to_rownames("X")
  colnames(abundance_data) <- gsub("X","",colnames(abundance_data))
  
  # Load metadata
  metadata_file <- paste0("Contig/data/", tolower(cohort_name), "/", cohort_name, ".contig.metadata.clean_0.75_0.03.csv")
...
# PA table generation for contigs
generate_contig_PA_table <- function(abundance_matrix, target_cpm = 0.5, min_reads_floor = 3L, M = 6L) {
...
# Load all contig cohorts
cohorts <- c("total", "US", "Italy")
cohort_data <- list()

for (cohort in cohorts) {
  tryCatch({
    cohort_data[[cohort]] <- load_contig_cohort_data(cohort)
    pa_table <- generate_contig_PA_table(cohort_data[[cohort]]$abundance)
    cohort_data[[cohort]]$PA <- pa_table
    cat("✅", cohort, "contig cohort loaded:", nrow(cohort_data[[cohort]]$abundance), "contigs,", 
        dim(pa_table)[1], "PA features,", ncol(cohort_data[[cohort]]$abundance), "samples\n")
  }, error = function(e) {
    cat("❌", cohort, "contig cohort failed to load:", e$message, "\n")
    cohort_data[[cohort]] <<- NULL
  })
}

cat("\n=== CONTIG PA ANALYSIS: ALL 5 MODELS ===\n")

# Contig PA Model execution function
run_contig_pa_model <- function(cohort_name, model_name, model_func, n_sample = 20) {
  if (is.null(cohort_data[[cohort_name]])) {
    cat("⏭️", cohort_name, model_name, "- cohort not available\n")
    return(NULL)
  }
  
  output_file <- paste0("Orf_Contig_Phrog_compositional/results/", cohort_name, "_contig_PA_", model_name, ".csv")
  ...
  cat("🔄", cohort_name, model_name, "- running", n_sample, "contigs...\n")
...
# Define contig PA models
contig_pa_models <- list(
...
# Run all contig PA models for all cohorts
contig_pa_results_summary <- data.frame()

for (cohort in names(cohort_data)) {
  if (!is.null(cohort_data[[cohort]])) {
    for (model_name in names(contig_pa_models)) {
      n_sample <- if (grepl("brms", model_name)) 8 else 
                  if (grepl("GEE", model_name)) 12 else 15
      
      result_count <- run_contig_pa_model(cohort, model_name, contig_pa_models[[model_name]], n_sample)
      
      if (!is.null(result_count)) {
        contig_pa_results_summary <- rbind(contig_pa_results_summary, 
          data.frame(cohort = cohort, model = model_name, results = result_count, type = "Contig_PA"))
      }
    }
  }
}

cat("\n=== CONTIG ABUNDANCE ANALYSIS: ALL 4 MODELS ===\n")

# Contig abundance model execution function
run_contig_abundance_model <- function(cohort_name, model_name, model_func, n_sample = 12) {
  if (is.null(cohort_data[[cohort_name]])) {
    cat("⏭️", cohort_name, model_name, "- cohort not available\n")
    return(NULL)
  }
  
  output_file <- paste0("Orf_Contig_Phrog_compositional/results/", cohort_name, "_contig_abundance_", model_name, ".csv")
  ...
  # Sample contigs
  sample_contigs <- head(rownames(cohort_data[[cohort_name]]$abundance), n_sample)
  sample_abundance <- cohort_data[[cohort_name]]$abundance[sample_contigs, ]
...
# Define contig abundance models
contig_abundance_models <- list(
...
# Run all contig abundance models for all cohorts
contig_abundance_results_summary <- data.frame()

for (cohort in names(cohort_data)) {
  if (!is.null(cohort_data[[cohort]])) {
    for (model_name in names(contig_abundance_models)) {
      n_sample <- if (grepl("limma", model_name)) 15 else 10
      
      result_count <- run_contig_abundance_model(cohort, model_name, contig_abundance_models[[model_name]], n_sample)
      
      if (!is.null(result_count)) {
        contig_abundance_results_summary <- rbind(contig_abundance_results_summary, 
          data.frame(cohort = cohort, model = model_name, results = result_count, type = "Contig_Abundance"))
      }
    }
  }
}
...
all_contig_results_summary <- rbind(contig_pa_results_summary, contig_abundance_results_summary)

# Create comprehensive summary
cat("\n=== CONTIG ANALYSIS COMPREHENSIVE RESULTS SUMMARY ===\n")
print(all_contig_results_summary)

# Save summary
write.csv(all_contig_results_summary, "Orf_Contig_Phrog_compositional/results/all_contig_models_execution_summary.csv", row.names = FALSE)

# Final file listing
all_files <- list.files("Orf_Contig_Phrog_compositional/results", pattern = "*contig*.csv", full.names = FALSE)
model_files <- all_files[grepl("_contig_PA_|_contig_abundance_", all_files)]

cat("\n=== FINAL CONTIG MODEL RESULTS SUMMARY ===\n")
cat("Total contig result files created:", length(model_files), "\n")
cat("Contig PA model files:", sum(grepl("_contig_PA_", model_files)), "\n")
cat("Contig abundance model files:", sum(grepl("_contig_abundance_", model_files)), "\n")

# Create final comprehensive summary
final_contig_summary <- data.frame(
  ...
  file_size_mb = round(file.info(paste0("Orf_Contig_Phrog_compositional/results/", model_files))$size / (1024*1024), 3)
)

write.csv(final_contig_summary, "Orf_Contig_Phrog_compositional/results/comprehensive_contig_model_summary.csv", row.names = FALSE)

cat("\n✅ ALL CONTIG MODELS EXECUTION COMPLETED! ✅\n")
cat("Total contig models attempted:", nrow(all_contig_results_summary), "\n")
cat("Successful contig completions:", sum(all_contig_results_summary$results > 0), "\n")
cat("Ready for Celiac_Phage_Analysis_Report_Final.html update!\n")
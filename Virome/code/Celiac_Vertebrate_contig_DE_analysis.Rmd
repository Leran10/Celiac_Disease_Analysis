---
title: "Celiac_Vertebrate_contig_DE_analysis"
...
# load the contig table and phage contigs
```{r}
...
# prepare total vertebrate count table with 75% mapping filtering + 3% prevelance filtering
```{r}
...
# 709 (975) contigs become 0 through all samples after the 32 samples were removed from the above step
...
# "01_GEMM_006_60M" "01_GEMM_060_18M" "01_GEMM_061_84M" "01_GEMM_102_12M" "01_GEMM_109_48M" "02_GEMM_006_30M" "02_GEMM_034_12M"
...
vertebrate_contig_abundance.clean <- vertebrate_contig_abundance.clean %>%
                       select(-c("01_GEMM_006_60M","01_GEMM_060_18M","01_GEMM_061_84M","01_GEMM_102_12M","01_GEMM_109_48M","02_GEMM_006_30M","02_GEMM_034_12M"))
dim(vertebrate_contig_abundance.clean)
# 3343  296
...
metadata.clean <- vertebrate_contig_count.table_0.75[1:27] %>%
                  filter(!Row.names %in% c("01_GEMM_006_60M","01_GEMM_060_18M","01_GEMM_061_84M","01_GEMM_102_12M","01_GEMM_109_48M","02_GEMM_006_30M","02_GEMM_034_12M")) %>%
                  column_to_rownames("Row.names")
dim(metadata.clean)
# 296  26
...
# prepare US vertebrate count table with 75% mapping filtering + 3% prevelance filtering
```{r}
library(data.table)
...
#"01_GEMM_006_60M" "01_GEMM_016_12M" "01_GEMM_060_18M" "01_GEMM_061_84M" "01_GEMM_102_12M" "01_GEMM_109_48M"

# remove the two 0 counts samples
# we will have to remove sample 02_GEMM_021_36M because it becomes 0 counts at this step
US_vertebrate_contig_abundance.clean <- US_vertebrate_contig_abundance.clean %>%
                       select(-c("01_GEMM_006_60M","01_GEMM_016_12M","01_GEMM_060_18M","01_GEMM_061_84M","01_GEMM_102_12M","01_GEMM_109_48M"))
dim(US_vertebrate_contig_abundance.clean)
# 3416  187
...
US_metadata.clean <- US_vertebrate_contig_count.table_0.75[1:27] %>%
                  filter(!Row.names %in% c("01_GEMM_006_60M","01_GEMM_016_12M","01_GEMM_060_18M","01_GEMM_061_84M","01_GEMM_102_12M","01_GEMM_109_48M")) %>%
                  column_to_rownames("Row.names")
...
# prepare Italy vertebrate count table with 75% mapping filtering + 3% prevelance filtering
```{r}
# library(data.table)
# 
# # filter contig count table to only keep vertebrate contigs
...
vertebrate_contig_total_PA_heatmaps$`Dx × TP interactions`
...
ggsave(vertebrate_contig_US_PA_heatmaps$`TP (CONTROL)`,file = "~/Downloads/US/US_vertebrate_contig_PA_results_everything_TP_CONTROL.pdf",dpi = 600, width = 6, height = 6)
...
write.csv(vertebrate_contig_Italy_PA.everything,"~/Downloads/Italy/Italy_vertebrate_contig_PA_results_everything.csv",row.names = FALSE)
...
ggsave(vertebrate_contig_Italy_PA_heatmaps$`TP (CONTROL)`,file = "~/Downloads/Italy/Italy_vertebrate_contig_PA_results_everything_TP_CONTROL.pdf",dpi = 600, width = 6, height = 3)


ggsave(vertebrate_contig_Italy_PA_heatmaps$`TP (CELIAC)`,file = "~/Downloads/Italy/Italy_vertebrate_contig_PA_results_everything_TP_CELIAC.pdf",dpi = 600, width = 6, height = 2.5)

ggsave(vertebrate_contig_Italy_PA_heatmaps$`Between groups`,file = "~/Downloads/Italy/Italy_vertebrate_contig_PA_results_everything_Between_groups.pdf",dpi = 600, width = 6, height = 6)

ggsave(vertebrate_contig_Italy_PA_heatmaps$`Dx × TP interactions`,file = "~/Downloads/Italy/Italy_vertebrate_contig_PA_results_everything_interactions.pdf",dpi = 600, width = 6, height = 6)
...
# Abundance model - Negative Binomial GLMM
...
View(vertebrate_contig_total_abundance.adjusted.results %>% filter(p.adj < 0.05))
...
write.csv(vertebrate_contig_total_abundance.everything,"~/Downloads/total/total_vertebrate_contig_abundance_results_everything.csv",row.names = FALSE)
...
vertebrate_contig_total_abundance.everything <- read.csv("~/Downloads/total/total_vertebrate_contig_abundance_results_everything.csv")
...
ggsave(vertebrate_contig_total_abundance_heatmaps$`TP (CONTROL)`,file = "~/Downloads/total/total_vertebrate_contig_abundance_results_everything_TP_CONTROL.pdf",dpi = 600, width = 6, height = 3)


ggsave(vertebrate_contig_total_abundance_heatmaps$`TP (CELIAC)`,file = "~/Downloads/total/total_vertebrate_contig_abundance_results_everything_TP_CELIAC.pdf",dpi = 600, width = 6, height = 2.5)

ggsave(vertebrate_contig_total_abundance_heatmaps$`Between groups`,file = "~/Downloads/total/total_vertebrate_contig_abundance_results_everything_Between_groups.pdf",dpi = 600, width = 6, height = 3)

ggsave(vertebrate_contig_total_abundance_heatmaps$`Dx × TP interactions`,file = "~/Downloads/total/total_vertebrate_contig_abundance_results_everything_interactions.pdf",dpi = 600, width = 6, height = 3)
...
write.csv(vertebrate_contig_US_abundance.everything,"~/Downloads/US/US_vertebrate_contig_abundance_results_everything.csv",row.names = FALSE)
...
vertebrate_contig_US_abundance.everything <- read.csv("~/Downloads/US/US_vertebrate_contig_abundance_results_everything.csv")
...
vertebrate_contig_US_abundance_heatmaps$`Between groups`
...
# 
# 
ggsave(vertebrate_contig_US_abundance_heatmaps$`TP (CELIAC)`,file = "~/Handley Lab Dropbox/16S/Celiac/Vertebrate_contigs/US/US_vertebrate_contig_abundance_results_everything_TP_CELIAC.pdf",dpi = 600, width = 3, height = 2.5)
# 
...
ggsave(vertebrate_contig_US_abundance_heatmaps$`TP (CONTROL)`,file = "~/Downloads/US/US_vertebrate_contig_abundance_results_everything_TP_CONTROL.pdf",dpi = 600, width = 6, height = 3)


ggsave(vertebrate_contig_US_abundance_heatmaps$`TP (CELIAC)`,file = "~/Downloads/US/US_vertebrate_contig_abundance_results_everything_TP_CELIAC.pdf",dpi = 600, width = 6, height = 2.5)

ggsave(vertebrate_contig_US_abundance_heatmaps$`Between groups`,file = "~/Downloads/US/US_vertebrate_contig_abundance_results_everything_Between_groups.pdf",dpi = 600, width = 6, height = 3)

ggsave(vertebrate_contig_US_abundance_heatmaps$`Dx × TP interactions`,file = "~/Downloads/US/US_vertebrate_contig_abundance_results_everything_interactions.pdf",dpi = 600, width = 6, height = 3)
...
vertebrate_contig_Italy_abundance_nb_results <- vertebrate_contig_Italy_abundance_long %>%
  ...
  mutate(
    model = map(data, ~ {
      tryCatch({
        glmmTMB(count ~ Dx.Status * onset_timeline_combined + Sex + Age.at.Gluten.Introduction..months. + 
                HLA.Category + feeding_first_year + Delivery.Mode  + offset(offset), # + (1 | patientID)
                family = nbinom2, data = .x)
      }, error = function(e) NULL)
    }),
    tidy_results = map(model, ~ {

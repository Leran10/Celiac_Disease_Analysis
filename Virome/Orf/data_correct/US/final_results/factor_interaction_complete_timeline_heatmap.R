...
cat("=== COMPLETE TIMELINE FACTOR MODEL INTERACTION HEATMAP ===\n")
...
# Load the overlapping ORFs (529 ORFs significant in all 3 interaction timepoints)
overlap_orfs <- read.csv("all_timepoints_overlapping_orfs.csv")
orf_list <- overlap_orfs$ORF
...
if(nrow(heatmap_matrix_clean) > 0) {
  ...
  png("factor_interaction_complete_timeline_heatmap.png", 
  ...
  pheatmap(heatmap_matrix_clean,
           ...
           main = paste("US Cohort: Factor Model -", nrow(heatmap_matrix_clean), 
  ...
  if(nrow(heatmap_matrix_clean) > 200) {
    ...
    png("factor_interaction_complete_timeline_top200_heatmap.png", 
        width = 2400, height = max(800, n_top * 15), res = 150)
    
    pheatmap(top_heatmap_matrix,
             cluster_rows = TRUE, cluster_cols = FALSE, scale = "none",
             color = colors, breaks = seq(color_limits[1], color_limits[2], length.out = 101),
             main = paste("US Cohort: Factor Model - Top", n_top, 

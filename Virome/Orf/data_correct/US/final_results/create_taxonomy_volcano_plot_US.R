#!/usr/bin/env Rscript
# Create Volcano Plot with Taxonomic Coloring - US Cohort
# Author: Claude AI
# Date: 2025-08-06
...
cat("=== CREATING TAXONOMY VOLCANO PLOT - US COHORT ===\n")
...
# Load limma results - US cohort
limma_results <- read.csv("US.limma_model_res.csv", row.names = 1)
cat("Loaded US cohort limma results with", nrow(limma_results), "ORFs\n")
...
cat("Significant limma results:", nrow(significant_results), "ORFs\n")
...
p <- ggplot(plot_data, aes(x = logFC_flipped, y = neg_log10_pval)) +
  ...
  labs(
    title = "Volcano Plot: US Cohort ORFs Colored by Taxonomy",
...
ggsave("taxonomy_volcano_plot_US.png", p, width = 14, height = 9, dpi = 300, bg = "white")
cat("Volcano plot saved as 'taxonomy_volcano_plot_US.png'\n")

# Also save as PDF
ggsave("taxonomy_volcano_plot_US.pdf", p, width = 14, height = 9, bg = "white")
cat("Volcano plot also saved as 'taxonomy_volcano_plot_US.pdf'\n")
...
# Save merged data
write.csv(merged_data, "limma_results_with_taxonomy_US.csv", row.names = FALSE)
cat("Merged data saved as 'limma_results_with_taxonomy_US.csv'\n")
...
summary_stats <- data.frame(
  ...
  Count = c(nrow(limma_results), nrow(merged_data),
...
write.csv(summary_stats, "taxonomy_analysis_summary_US.csv", row.names = FALSE)
cat("Summary statistics saved as 'taxonomy_analysis_summary_US.csv'\n")

# Print summary
cat("\n=== TAXONOMY VOLCANO PLOT ANALYSIS COMPLETED - US COHORT ===\n")
cat("Generated files:\n")
cat("- taxonomy_volcano_plot_US.png: Main volcano plot with taxonomic coloring\n")
cat("- taxonomy_volcano_plot_US.pdf: PDF version of the plot\n")
cat("- limma_results_with_taxonomy_US.csv: Merged dataset with taxonomy annotations\n")
cat("- taxonomy_analysis_summary_US.csv: Summary statistics\n")
